...
export const createUserWorkflow = createWorkflow(
	...
	function (input: CreateUserWorkflowInput) {
		/* create user */
		const restaurantUser = when(
			input,
			(input) => input.user.actor_type === "restaurant"
		).then(() => {
		...
		const driverUser = when(
			input,
			(input) => input.user.actor_type === "driver"
		).then(() => {
			return createDriverStep(input.user as CreateDriverInput);
		});

		const { user, authUserInput } = transform(
			{ input, restaurantUser, driverUser },
			(data) => {
		...
		return new WorkflowResponse(user as any);
	}
);
